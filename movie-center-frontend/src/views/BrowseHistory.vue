...
const browseRecords = ref<BrowseRecord[]>([])
...
const handleTableChange: TableProps['onChange'] = (pagination, filters, sorter) => {
  ...
  if (sorter.order) {
    ...
    // 没有排序时，恢复原始顺序（按browse_id排序）
    combinedData.value.sort((a, b) => a.browse_id - b.browse_id)
  }
}
...
async function fetchRecords() {
  ...
  try {
    // 1. 获取浏览记录
    const historyRes = await getBrowseHistory(authStore.user?.userid)
    browseRecords.value = historyRes.data.data

    // 2. 获取所有电影详情
    const movieIds = [...new Set(browseRecords.value.map(r => r.movie_id))]
    ...
    combinedData.value = browseRecords.value.map(record => ({
      ...record,
